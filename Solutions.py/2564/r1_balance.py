...
amt_log, ifo_log, amt_add = int(input()), dict(()), 0
for each_log in range(amt_log): ifo_log[each_log+1] = [int(a) for a in input().split()]
def amt_sum(ifo_tree):
    global amt_add
    amt_max = 0
    if ifo_tree[0] == 1 and ifo_tree[2] == 1:
        ...
        tmp_1, tmp_2 = amt_sum(ifo_log[ifo_tree[1]]), ifo_tree[3]
    elif ifo_tree[0] == 1 and ifo_tree[2] == 0:
        tmp_1, tmp_2 = ifo_tree[1], amt_sum(ifo_log[ifo_tree[3]])
    elif ifo_tree[0] == 0 and ifo_tree[2] == 0:
        tmp_1, tmp_2 = amt_sum(ifo_log[ifo_tree[1]]), amt_sum(ifo_log[ifo_tree[3]])
    amt_add += abs(tmp_1 - tmp_2)
    amt_max = max(tmp_1, tmp_2) * 2
    return amt_max
amt_sum(ifo_log[1])
print(amt_add)
